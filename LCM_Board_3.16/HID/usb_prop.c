...
void Joystick_Reset(void)
{
  ...
  /* Initialize Endpoint 1 */
     SetEPType(ENDP1, EP_INTERRUPT);//EP1为中断端点
     SetEPRxAddr(ENDP1, ENDP1_RXADDR);//设置为接收端点
     SetEPRxCount(ENDP1, nReportCnt);	//最大字节数
     SetEPRxStatus(ENDP1, EP_RX_VALID);
     //SetEPTxStatus(ENDP1, EP_TX_DIS);

  SetEPType(ENDP2, EP_INTERRUPT);//为中断端点
  SetEPTxAddr(ENDP2, ENDP2_TXADDR);	//设置为发送端点
  SetEPTxCount(ENDP2, nReportCnt);	//最大字节数
 // SetEPTxStatus(ENDP2, EP_TX_DIS);
  SetEPTxStatus(ENDP2, EP_TX_NAK);	//发送禁止
