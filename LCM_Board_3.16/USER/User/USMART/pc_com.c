...
{
		u16 parmer;
		printf("SSD2828ReadReg:");
		parmer = SSD2828ReadReg(reg);	//SSD2828ReadReg
	 if(parmer == 0x2828){
		USB_SendString("0x2828");
		printf("0x%X\r\n",parmer);
	 }else{printf("0x%X\r\n",parmer);}
}
...
void PC_DCS_Long_Write_FIFO(u16 NUM,char *P)
{
	...
	DCS_Long_Write_FIFO(NUM,mbuffer);
}
...
void PC_Generic_Long_Write_FIFO(u16 NUM,char *P)
{
	...
	Generic_Long_Write_FIFO(NUM,mbuffer);
}
